...
'  Reprezentuje AKTUALNIE zalogowanego usera. Multi-user (wielu userow w
'  Registry) obsluguje osobny modul mod_UsersRegistrySync (post-M3.3
'  refactor 2026-07-26 - patrz ADR-001 aktualizacja).
'
'  Detekcja roli: IsUserManager() porownuje EmailKierownika z EmailHandlowca.
'  Patrz: DECISIONS.md (ADR-001, ADR-002, ADR-008)
'         doc_v2/diagrams/04_data_model.md (sekcja 1 - UserCache)
' ============================================================================
...
' Uzywane przez frm_Setup.btn_Save (posrednio przez mod_UsersRegistrySync)
' oraz mod_UsersRegistrySync.LoadUserFromRegistry (po SwitchUser).
...
' Czysci wszystkie pola UserCache (wszystkie key-value pairs w kolumnach A/B).
' Uzywane przez mod_UsersRegistrySync.PrepareForNewUser (przed frm_Setup dla
' nowego usera) i przez LoadUserFromRegistry (przed loadem nowego usera do
' UserCache przy SwitchUser). Bez ThisWorkbook.Save/SyncToFile - caller
' sam decyduje kiedy save (typowo po ClearUserCache nastepuje SaveUserData
' ktore juz robi save+sync).
Public Sub ClearUserCache()
    Dim ws As Worksheet
    Set ws = ThisWorkbook.Worksheets(SHEET_NAME)
