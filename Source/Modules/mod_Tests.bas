...
' ============================================================================
'  mod_Tests
'  Smoke testy dla modułów Foundation. Uruchamiaj z Immediate Window:
...
' nie tylko istnienie w UserForms. Bez testu regresja mogłaby cicho
' przywrócić bug (blokada re-Show po Me.Hide).
'
' Nie testujemy visible=True case - wymagałoby to Show vbModal blokującego
...
' na końcu kasuje. NIE dotyka _CurrentUserID żeby nie zepsuć sesji.
...
' Pisze do ws_DataCache jeden tymczasowy rekord i kasuje na końcu.
Public Sub Test_mod_DataCacheSync()
    ...
    ' Cleanup sent rekordu bezpośrednio (bypass API - DeleteRecord odmawia
...
' NIE wysyła maila - testuje tylko DetermineRecipient (czysta funkcja).
' Manipuluje EmailKierownika w Registry żeby sprawdzić obie gałęzie
' decision diamond, na końcu przywraca oryginalną wartość.
...
' (test nie kasuje). Usuń manualnie jeśli nie potrzeba historii.
Public Sub Test_MultiUser()
    ...
    ' SwitchUser back do poprzedniego (jeśli był)
