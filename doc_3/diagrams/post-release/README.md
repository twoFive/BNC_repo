...
> **Uwaga**: te diagramy reprezentują **planowany** stan po Release (M7), **NIE aktualny** stan kodu. Aktualny (po M5) jest w nadrzędnym katalogu `doc_3/diagrams/`.

## Kiedy używać których diagramów

| Sytuacja | Folder |
|---|---|
| Dyskusja "jak architektura wygląda **teraz**" | `doc_3/diagrams/` (po M5) |
| Pytanie "jak będzie wyglądać **finalna wersja**" | `doc_3/diagrams/post-release/` (ten folder) |
...
3. **Po release v1.0.0** — wykonaj `mv post-release/*.jpg ../` (zastąp wersję po M5), zaktualizuj `doc_3/diagrams/README.md`, usuń `post-release/` (cele osiągnięty).

## Regeneracja

Identyczny pipeline jak w nadrzędnym `doc_3/diagrams/README.md` — Chrome headless → PowerShell System.Drawing.

```bash
cd doc_3/diagrams/post-release
